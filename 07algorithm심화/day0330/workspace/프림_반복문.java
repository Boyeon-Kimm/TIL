...
public class 프림_반복문 {
	...
	public static void main(String[] args) {
		...
		// dist는 아주 큰 값으로 갱신을 해라
		for(int i = 0; i < V; i++) {
			dist[i] = INF;
		}
		
		Arrays.fill(dist, INF); // 위나 얘나 동작은 똑같음.
		
		// 임의의 한 점을 선택해서 돌려야한다
		// 아무거나 선택해도 괜찮음
		dist[0] = 0; // 0번 정점을 가지고 시작할거야
		p[0] = -1; // 초기화
		
		int ans = 0; // 정답 담을 변수
		
		// 프림을 동작시키겠다
		// V만큼 돌아도 상관없음, 하지만 마지막에 방문하지 않은 배열이 1개가 남아있다면
		// 마지막 1개는 굳이 반복하지 않아도 되기 때문에 V - 1
		for(int i = 0; i < V - 1; i++) {
			// 1. 내가 가장 작은 값을 뽑겠다
			int min = INF;
			int idx = -1;
